...
4. Добавляем конфигурацию для запуска консьюмеров в файле `supervisor/consumer.conf`
    ...
    ```
5. В классе `App\Controller\Amqp\AddFollowers\Consumer` в методе `handle` убираем ожидание 1 секунды
6. Запускаем контейнеры командой `docker-compose up -d`
7. Проверяем в RabbitMQ, что консьюмер запущен
8. Выполняем запрос Add user v2 из Postman-коллекции v9
9. Выполняем запрос Add followers из Postman-коллекции v9 с параметрами `async` = 1 и `count` = 1000, проверяем, что
